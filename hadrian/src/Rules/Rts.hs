...
copyMmtkGhcStatic stage target = do
    -- Copy the vanilla library, and symlink the rest to it.
    vanillaLibFile <- rtsMmtkLibrary stage vanilla
    if target == vanillaLibFile
    then copyFile' mmtkLibPath target
    else createFileLink (takeFileName vanillaLibFile) target
  where
    mmtkLibPath = "rts/mmtk/mmtk/target/debug/libmmtk_ghc.a"
...
rtsMmtkLibrary :: Stage -> Way -> Action FilePath
rtsMmtkLibrary stage way = do
    suf     <- libsuf stage way
    rtsPath <- rtsBuildPath stage
    return $ rtsPath -/- "libCmmtk_ghc" ++ suf
       ...
       | otherwise    -> do
        ...
        staticMmtks <- do
            ways <- interpretInContext (stageContext stage)
                                       (getLibraryWays <> getRtsWays)
            let staticWays = Set.toList $ Set.filter (not . wayUnit Dynamic) ways
            mapM (rtsMmtkLibrary stage) staticWays

        return $ concat [ headers, dynLibffis, staticLibffis, staticMmtks ]
