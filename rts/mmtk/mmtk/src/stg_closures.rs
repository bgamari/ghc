...
impl fmt::Debug for StgTSOBlockInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        unsafe { write!(f, "StgTsoBlockInfo({:?})", self.fd) }
    }
}
                ...
                RET_FUN => unsafe {
                    ...
                    match fun_info.f.fun_type {
                        ...
                        ARG_GEN_BIG => {
                            ...
                            let mut size =
                                bitmap.size * size_of::<StgWord>() + size_of::<StgStackFrame>();
                            size += size_of::<StgRetFunFrame>();
                            self.current = offset_bytes(self.current, size as isize);
...
union FinalizerFn {
    ...
    // ^ (ptr)
    pub with_env: *const extern "C" fn(*mut u8, *mut u8), // ^ (eptr, ptr)
}
